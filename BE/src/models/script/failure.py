...
def predict_failure(data_list):
    ...
    # Prediksi Failure
    failure_pred = failure_model.predict(features)
    faulure_proba = failure_model.predict_proba(features)
    
    # Kembalikan sebagai list agar JSON serializable
    return {
        "predictions": failure_pred.tolist(),
        "probabilities": faulure_proba.tolist()
    }
...
if __name__ == "__main__":
    ...
    try:
        ...
        output = predict_failure(data_list)
